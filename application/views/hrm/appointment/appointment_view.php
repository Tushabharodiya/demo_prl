<?php
    $isAppointmentAdd = checkPermission(HRM_APPOINTMENT_ALIAS, "can_add");
    $isAppointmentView = checkPermission(HRM_APPOINTMENT_ALIAS, "can_view");
    $isAppointmentEdit = checkPermission(HRM_APPOINTMENT_ALIAS, "can_edit");
    $isAppointmentDelete = checkPermission(HRM_APPOINTMENT_ALIAS, "can_delete");
...
<div class="nk-content-wrap">
    <div class="nk-block nk-block-lg">
                            ...
                            <ul class="nk-block-tools g-3">
                                                        ...
                                                        <div class="row gx-6 gy-3">
                                                            <div class="col-12">
                                                                <div class="form-group">
                                                                    ...
                                                                    <select class="form-control form-select" id="search-status" name="search_appointment_status" data-placeholder="Select a status">
                                                                        <?php $str='';
                                                                            if(!empty($sessionAppointmentStatus == 'all')){
                                                                                $str.='selected';
                                                                        } ?> <option value="all"<?php echo $str; ?>>All</option>
                                                                        <?php $str='';
                                                                            if(!empty($sessionAppointmentStatus == 'active')){
                                                                                $str.='selected';
                                                                        } ?> <option value="active"<?php echo $str; ?>>Active</option>
                                                                        <?php $str='';
                                                                            if(!empty($sessionAppointmentStatus == 'inactive')){
                                                                                $str.='selected';
                                ...
                                <?php if(!empty($this->session->userdata['user_role'])){ ?>
                                    <?php if($this->session->userdata['user_role'] == "Super"){ ?>
                                        <li><a href="<?php echo base_url(); ?>view-trash-appointment" class="btn btn-white btn-outline-light"><em class="icon ni ni-trash-fill"></em><span>Trash <sup><?php echo $countAppointmentTrash; ?></sup></span></a></li>
                                    <?php } ?>
                                <?php } ?>
                                <?php if($isAppointmentAdd){ ?>
                                    <li class="nk-block-tools-opt d-block d-sm-block">
                                        <a href="<?php echo base_url(); ?>new-appointment" class="btn btn-primary"><em class="icon ni ni-plus"></em></a>
                                    </li>
                                <?php } ?>
                            </ul>
                        </div>
                ...
                <form action="" method="post" class="form-validate is-alter" enctype="multipart/form-data">
                    <div class="form-group">
                        <div class="form-control-wrap">
                            <input type="text" class="form-control form-control-lg" name="search_appointment" value="<?php if(!empty($sessionAppointment)){ echo $sessionAppointment; } ?>" placeholder="Search..." autocomplete="off">
        ...
        <?php if(!empty($this->session->userdata('session_appointment_trash_success'))){ ?>
            <div class="alert alert-success alert-icon">
                <em class="icon ni ni-alert-circle"></em> 
                <?php echo $this->session->userdata('session_appointment_trash_success'); $this->session->unset_userdata('session_appointment_trash_success'); ?>
            </div>
        <?php } ?>
                                    ...
                                    <?php if($isAppointmentView){ ?>
                                        <?php if(!empty($viewAppointment)){ ?>
                                            <tbody>
                                                ...
                                                <tr class="tb-tnx-item">
                                                    ...
                                                    <td class="nk-tb-col">
                                                        <div class="user-card">
                                                            ...
                                                            <?php } ?>
                                                            <a href="<?php echo base_url(); ?>detail-appointment/<?php echo urlEncodes($data['appointment_id']); ?>">
                                                    ...
                                                    <td class="nk-tb-col nk-tb-col-tools">
                                                        <ul class="nk-tb-actions gx-1">
                                                            <?php if($isAppointmentEdit){ ?>
                                                                <?php if($data['employeeData']['employee_status'] == 'active'){ ?>
                                                                    <li class="nk-tb-action">
                                                                        <a href="<?php echo base_url(); ?>edit-appointment/<?php echo urlEncodes($data['appointment_id']); ?>" class="btn btn-trigger btn-icon" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit">
                                                            ...
                                                            <?php if(!empty($this->session->userdata['user_role'])){ ?>
                                                                <?php if($this->session->userdata['user_role'] == "Super"){ ?>
                                                                    <li class="nk-tb-action">
                                                                        <a data-bs-toggle="modal" data-bs-target="#trashModal<?php echo urlEncodes($data['appointment_id']);?>" class="btn btn-trigger btn-icon" data-toggle="tooltip" data-placement="top" title="Trash">
                                                ...
                                                <div class="modal fade" tabindex="-1" id="trashModal<?php echo urlEncodes($data['appointment_id']);?>">
                                                    <div class="modal-dialog modal-dialog-top" role="document">
                                                        <div class="modal-content">
                                                            <div class="modal-header">
                                                                <h5 class="modal-title">Trash Appointment</h5>
                                                            ...
                                                            <div class="modal-footer bg-light">
                                                                <span class="sub-text"><a href="<?php echo base_url(); ?>trash-appointment/<?php echo urlEncodes($data['appointment_id']); ?>" class="btn btn-sm btn-warning submitBtn">Trash</a></span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
...
<script>
    document.getElementById('search-status').addEventListener('change', function() {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: { search_appointment_status: selectedStatus },
            success: function() {
