<?php
    $isExperienceAdd = checkPermission(HRM_EXPERIENCE_ALIAS, "can_add");
    $isExperienceView = checkPermission(HRM_EXPERIENCE_ALIAS, "can_view");
    $isExperienceEdit = checkPermission(HRM_EXPERIENCE_ALIAS, "can_edit");
    $isExperienceDelete = checkPermission(HRM_EXPERIENCE_ALIAS, "can_delete");
...
<div class="nk-content-wrap">
    <div class="nk-block nk-block-lg">
                            ...
                            <ul class="nk-block-tools g-3">
                                                        ...
                                                        <div class="row gx-6 gy-3">
                                                            <div class="col-12">
                                                                <div class="form-group">
                                                                    ...
                                                                    <select class="form-control form-select" id="search-status" name="search_experience_status" data-placeholder="Select a status">
                                                                        <?php $str='';
                                                                            if(!empty($sessionExperienceStatus == 'all')){
                                                                                $str.='selected';
                                                                        } ?> <option value="all"<?php echo $str; ?>>All</option>
                                                                        <?php $str='';
                                                                            if(!empty($sessionExperienceStatus == 'active')){
                                                                                $str.='selected';
                                                                        } ?> <option value="active"<?php echo $str; ?>>Active</option>
                                                                        <?php $str='';
                                                                            if(!empty($sessionExperienceStatus == 'inactive')){
                                                                                $str.='selected';
                                ...
                                <?php if(!empty($this->session->userdata['user_role'])){ ?>
                                    <?php if($this->session->userdata['user_role'] == "Super"){ ?>
                                        <li><a href="<?php echo base_url(); ?>view-trash-experience" class="btn btn-white btn-outline-light"><em class="icon ni ni-trash-fill"></em><span>Trash <sup><?php echo $countExperienceTrash; ?></sup></span></a></li>
                                    <?php } ?>
                                <?php } ?>
                                <?php if($isExperienceAdd){ ?>
                                    <li class="nk-block-tools-opt d-block d-sm-block">
                                        <a href="<?php echo base_url(); ?>new-experience" class="btn btn-primary"><em class="icon ni ni-plus"></em></a>
                                    </li>
                                <?php } ?>
                            </ul>
                        </div>
                ...
                <form action="" method="post" class="form-validate is-alter" enctype="multipart/form-data">
                    <div class="form-group">
                        <div class="form-control-wrap">
                            <input type="text" class="form-control form-control-lg" name="search_experience" value="<?php if(!empty($sessionExperience)){ echo $sessionExperience; } ?>" placeholder="Search..." autocomplete="off">
        ...
        <?php if(!empty($this->session->userdata('session_experience_trash_success'))){ ?>
            <div class="alert alert-success alert-icon">
                <em class="icon ni ni-alert-circle"></em> 
                <?php echo $this->session->userdata('session_experience_trash_success'); $this->session->unset_userdata('session_experience_trash_success'); ?>
            </div>
        <?php } ?>
                                    ...
                                    <?php if($isExperienceView){ ?>
                                        <?php if(!empty($viewExperience)){ ?>
                                            <tbody>
                                                ...
                                                <tr class="tb-tnx-item">
                                                    ...
                                                    <td class="nk-tb-col">
                                                        <div class="user-card">
                                                            ...
                                                            <?php } ?>
                                                            <a href="<?php echo base_url(); ?>detail-experience/<?php echo urlEncodes($data['experience_id']); ?>">
                                                    ...
                                                    <td class="nk-tb-col nk-tb-col-tools">
                                                        <ul class="nk-tb-actions gx-1">
                                                            <?php if($isExperienceEdit){ ?>
                                                                <?php if($data['employeeData']['employee_status'] == 'active'){ ?>
                                                                    <li class="nk-tb-action">
                                                                        <a href="<?php echo base_url(); ?>edit-experience/<?php echo urlEncodes($data['experience_id']); ?>" class="btn btn-trigger btn-icon" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit">
                                                            ...
                                                            <?php if(!empty($this->session->userdata['user_role'])){ ?>
                                                                <?php if($this->session->userdata['user_role'] == "Super"){ ?>
                                                                    <li class="nk-tb-action">
                                                                        <a data-bs-toggle="modal" data-bs-target="#trashModal<?php echo urlEncodes($data['experience_id']);?>" class="btn btn-trigger btn-icon" data-toggle="tooltip" data-placement="top" title="Trash">
                                                ...
                                                <div class="modal fade" tabindex="-1" id="trashModal<?php echo urlEncodes($data['experience_id']);?>">
                                                    <div class="modal-dialog modal-dialog-top" role="document">
                                                        <div class="modal-content">
                                                            <div class="modal-header">
                                                                <h5 class="modal-title">Trash Experience</h5>
                                                            ...
                                                            <div class="modal-footer bg-light">
                                                                <span class="sub-text"><a href="<?php echo base_url(); ?>trash-experience/<?php echo urlEncodes($data['experience_id']); ?>" class="btn btn-sm btn-warning submitBtn">Trash</a></span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
...
<script>
    document.getElementById('search-status').addEventListener('change', function() {
        ...
        $.ajax({
            url: '<?= base_url('view-experience'); ?>',
            type: 'POST',
            data: { search_experience_status: selectedStatus },
            success: function() {
