...
<div class="nk-content-wrap">
    <div class="nk-block nk-block-lg">
                    ...
                    <div class="toggle-wrap nk-block-tools-toggle">
                        ...
                        <div class="toggle-expand-content" data-content="pageMenu">
                            <ul class="nk-block-tools g-3">
                                <li><a href="<?php if(!empty($sessionSopUserViewPreviousUrl)){ echo $sessionSopUserViewPreviousUrl; } ?>" class="btn btn-outline-light bg-white d-block d-sm-inline-flex"><em class="icon ni ni-arrow-left"></em></a></li>
                            </ul>
                        </div>
                    </div>
                </div>
                    ...
                    <form action="" method="post" class="form-validate is-alter" enctype="multipart/form-data">
                        <div class="form-group">
                            <div class="form-control-wrap">
                                <input type="text" class="form-control form-control-lg" name="search_sop_user_trash" value="<?php if(!empty($sessionSopUserTrash)){ echo $sessionSopUserTrash; } ?>" placeholder="Search..." autocomplete="off">
        ...
        <?php if(!empty($this->session->userdata('session_sop_user_restore_success'))){ ?>
            <div class="alert alert-success alert-icon">
                <em class="icon ni ni-alert-circle"></em> 
                <?php echo $this->session->userdata('session_sop_user_restore_success'); $this->session->unset_userdata('session_sop_user_restore_success'); ?>
            </div>
        <?php } ?>
        
        <?php if(!empty($this->session->userdata('session_sop_user_delete_success'))){ ?>
            <div class="alert alert-success alert-icon">
                <em class="icon ni ni-alert-circle"></em> 
                <?php echo $this->session->userdata('session_sop_user_delete_success'); $this->session->unset_userdata('session_sop_user_delete_success'); ?>
            </div>
        <?php } ?>
        
        <?php if(!empty($this->session->userdata('session_sop_user_delete_error'))){ ?>
            <div class="alert alert-danger alert-icon">
                <em class="icon ni ni-alert-circle"></em> 
                <?php echo $this->session->userdata('session_sop_user_delete_error'); $this->session->unset_userdata('session_sop_user_delete_error'); ?>
            </div>
        <?php } ?>
                                        ...
                                        <?php if(!empty($viewUser)){ ?>
                                            <tbody>
                                                    ...
                                                    <div class="modal-dialog modal-dialog-top" role="document">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body">
                                                                <p>Are you sure you want to restore <?php echo $data['userData']['user_name'];?>?</p>
                                                            </div>
                                                            <div class="modal-footer bg-light">
                                                                <span class="sub-text"><a href="<?php echo base_url(); ?>restore-sop-user/<?php echo urlEncodes($data['unique_id']); ?>" class="btn btn-sm btn-primary submitBtn">Restore</a></span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="modal fade" tabindex="-1" id="deleteModal<?php echo urlEncodes($data['unique_id']);?>">
                                                    <div class="modal-dialog modal-dialog-top" role="document">
                                                        <div class="modal-content">
                                                            <form action="<?php echo base_url(); ?>delete-sop-user/<?php echo urlEncodes($data['unique_id']); ?>" method="post" class="form-validate is-alter" enctype="multipart/form-data">
                                                                <div class="modal-header">
                                                                        ...
                                                                        <div class="col-md-12">
                                                                            <div class="form-group">
                                                                                <div class="form-label-group">
                                                                                    <label class="form-label" for="password">Are you sure you want to permanently delete <?php echo $data['userData']['user_name'];?>?</label>
                                                                                </div>
                                                                                <div class="form-control-wrap">
