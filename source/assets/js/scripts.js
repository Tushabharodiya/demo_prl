...
$(window).bind("load", function() {
    window.setTimeout(function() {
        $(".alert-icon").fadeTo(500, 0).slideUp(500, function(){
            $(this).remove();
        });
    }, 500);
});
...
$(document).ready(function() {
    $('.submitBtn').on('click', function() {
        ...
        if (isValid) {
            var $this = $(this);
            var submitBtnClass = $this.attr('class');
            $this.hide();
            var loadingHtml = '<button type="button" class="' + submitBtnClass + ' loadingBtn" disabled>' +
                              '<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>' +
                              '<span>Loading...</span></button>';
            $this.after(loadingHtml);
            $('.' + submitBtnClass + '.loadingBtn').show();
        } 
    });
});
